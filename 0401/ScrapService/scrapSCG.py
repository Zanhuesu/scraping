...
def runSQL(filename):
    ...
    for line in open(filename, 'r', encoding="utf8"): 
        ...
        # if ";" in strBuffer:
        #     line_tokens = strBuffer.split(";")
        #     beginpos = line_tokens[0].rfind('(')
        #     endpos = line_tokens[0].rfind(')')
        #     if beginpos <= endpos:
        #         sqlQuery = line_tokens[0] + ";"
        #         strBuffer = ';'.join(line_tokens[1:])
        #     elif line_tokens[1] != "":
        #         sqlQuery = line_tokens[0] + ";" + line_tokens[1] + ';'
        #         if (len(line_tokens) > 2):
        #             strBuffer = ';'.join(line_tokens[2:])
        #         else: 
        #             strBuffer = ""
        #     else:
        #         continue
...
def scrapSCGProduct():
    ...
    page_count = 931
    product_object= {}
    for i in range(232, page_count):
        browser.get(product_url + "?mpp=96&pg=" + str(i))
